                                    ...
                                    <form onSubmit={handleSubmit(edit)} enctype="multipart/form-data">
                                        <div class="row">
                                            <div class="form-group mb-3 col-7">
                                                ...
                                                <input type="text" name="title" class={`form-control border border-secondary ${errors.title ? 'is-invalid' : ''}`} defaultValue={item && item.title} placeholder="Article Title" id="posttitle" {...register('title')} />
                                                <div className="invalid-feedback">{errors.title?.message}</div>
                                            </div>
                                            <div class="form-group mb-3 col-5">
                                                <label for="postcategory">Category</label>
                                                <div>
                                                    <select name="news_category_id" class={`custom-select  ${errors.category ? 'is-invalid' : ''}`} defaultValue={item && item.news_category_id} id="inputGroupSelect01" style={{ "height": "35px" }}  {...register('category')}>
                                                        <option value="0" >Choose Category</option>

                                                        {categoryList.map((cat, index) => {
                                                            return (
                                                                <option key={index} value={cat._id} selected={item && item.news_category_id === cat._id ? "selected" : ""}>{cat.name}</option>
                                                            )
                                                        })}
                                                    </select>
                                        ...
                                        <div class="form-group mb-3">
                                            ...
                                            <textarea class={`form-control border border-secondary ${errors.content ? 'is-invalid' : ''}`} defaultValue={item && item.content} name="content" placeholder="Article Content" id="postcontent" {...register('content')}></textarea>
                                        ...
                                        <div className="text-center">
                                            ...
                                            <button type="submit" class="btn btn-primary" onClick={() => {
                                                ...
                                                unregister("category", { keepDefaultValue: true });
